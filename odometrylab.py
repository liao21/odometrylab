...
class StateMachine():
    ...
    def __init__(self):
        ...
        # If its unable to connect after 10 seconds, give up.  Want this to be a while so robot can init.
        self.TIMEOUT = 10
        self.STATE = States.ON_LINE        # Our initial state is LISTEN
        # We are running if our state machine is initialized
        self.RUNNING = True
        self.DIST = False
        ...
        try:
            with socketLock:
                self.sock = socket.create_connection(
                    (self.IP_ADDRESS, self.CONTROLLER_PORT), self.TIMEOUT)
...
class Sensing(threading.Thread):
    def __init__(self, socket):
        # MUST call this to make sure we setup the thread correctly
    ...
    def run(self):
        ...
        while self.RUNNING:
            ...
            with socketLock:  # !IMPORTANT: Threadsafe-ness
                #save bumper sensors to self.left/right_sensor
            ...
            print("Encoders --\tLeft: ", self.left_encoder,
